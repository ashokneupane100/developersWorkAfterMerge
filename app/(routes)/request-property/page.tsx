...
const areaUnits = ["Aana", "Ropani", "Dhur", "Kattha", "Bigha", "Hectare", "Square Feet", "Square Meter"];
...
const PropertyRequestForm: React.FC = () => {
    ...
    // Effect to scroll to top when success or error state changes
    useEffect(() => {
        if (success || error) {
            scrollToTop();
        }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!coordinates.lat || !coordinates.lng) return toast.error("Invalid location.");
        if (!formData.propertyType) return toast.error("Select a property type.");
        if (!formData.budget) return toast.error("Select a budget range.");
        ...
        try {
            ...
            const submitData = {
                ...
                road_width_unit: formData.roadWidthUnit,
            ...
            {success ? (
                ...
                <form onSubmit={handleSubmit} className="divide-y divide-gray-200">
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                            <div>
                                ...
                                >
                                    ...
                                    {propertyTypes?.map((type: { key: string }) => (
                                ...
                                <div className="flex items-center p-4 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300 bg-white">
                                    <label className="flex items-center w-full cursor-pointer">
                                        <div className="relative">
                                            ...
                                            />
                                            <div
                                                className={`relative w-12 h-6 flex items-center rounded-full transition-colors duration-300 ease-in-out ${formData.financing ? 'bg-green-500' : 'bg-gray-300'
                                                    }`}
                                            >
                                                <div
                                                    className={`absolute ml-1 bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ease-in-out ${formData.financing && 'right-0 mr-1'
                                                        }`}
                                                ></div>
                                            </div>
                                        ...
                                        <div className={`ml-auto px-3 py-1 rounded-full text-xs font-medium transition-colors duration-300 ${formData.financing
                        ...
                        {formData.propertyType === "Room/Flat" && (
                            <div className="mt-6">
                                <div className="p-4 rounded-md bg-gray-50 border border-gray-200">
