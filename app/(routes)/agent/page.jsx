...
export default function AgentDashboard() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        const { data: userRes, error: userErr } = await supabase.auth.getUser();
        if (userErr)
          console.warn("[Dashboard] getUser error:", userErr.message);
  ...
  // Mobile-first white bg; md+ gets colored tint
  const statusBadgeClasses = (statusRaw) => {
    const s = norm(statusRaw);
    const base =
      "text-xs px-2 py-1 border rounded-md shadow-sm bg-white md:shadow-none";
    switch (s) {
      case "approved":
        return `${base} text-emerald-700 border-emerald-200 md:bg-emerald-100 md:hover:bg-emerald-100`;
      case "pending":
        return `${base} text-amber-700 border-amber-200 md:bg-amber-100 md:hover:bg-amber-100`;
      case "rejected":
        return `${base} text-red-700 border-red-200 md:bg-red-100 md:hover:bg-red-100`;
      default:
        return `${base} text-gray-700 border-gray-200 md:bg-gray-100 md:hover:bg-gray-100`;
    }
  };
      ...
      <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="gradient-card stat-card border-l-4 border-l-blue-500">
          ...
          <CardContent>
            ...
            <div className="flex items-center text-[10px] sm:text-xs text-emerald-600 mt-1">
              <ArrowUpRight className="h-3 w-3 mr-1" />
              {loading ? "—" : "All-time"}
            </div>
          </CardContent>
              ...
              <div className="space-y-3 sm:space-y-4">
                {recentListings.map((listing) => {
                  ...
                  return (
                    <div
                      key={listing.id}
                      className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-white rounded-xl border hover:shadow-md transition-all"
                    >
