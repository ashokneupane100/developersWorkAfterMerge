  ...
  if (code) {
    ...
    try {
      ...
      if (error) {
        ...
        return NextResponse.redirect(new URL(`/login?error=AuthError&provider=${provider}`, request.url));
      }
      
      // For debugging
      console.log(`Successfully authenticated with ${data.user?.app_metadata?.provider || "unknown provider"}`);
