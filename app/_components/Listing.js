                ...
                >
                  ...
                  <span className="truncate">
                    ...
                    {searchTerm.toLowerCase() !==
                      location.toLowerCase().substring(0, searchTerm.length) &&
                      !location
                        .toLowerCase()
...
function Listing(props) {
  ...
  const {
    ...
    handleSearchClick,
    searchedAddress,
    setBathRoomsCount: parentSetBathRoomsCount,
    setRoomsCount: parentSetRoomsCount,
    setParkingCount: parentSetParkingCount,
    setPriceRange: parentSetPriceRange,
    setArea: parentSetArea,
    currentAction,
    setCurrentAction,
    propertyType,
    setPropertyType,
    ...
    isSearchPerformed: parentIsSearchPerformed,
    isFilterApplied: parentIsFilterApplied,
  ...
  const getFilteredListings = () => {
    ...
    if (parkingCount > 0) {
      filteredData = filteredData.filter(
        (item) => (item.parking || 0) >= parkingCount
      );
    }
  ...
  const getDisplayListings = () => {
    ...
    if (viewFilter === "sale")
      return filteredData.filter((item) => item.action === "Sell");
    if (viewFilter === "rent")
      return filteredData.filter((item) => item.action === "Rent");
  ...
  const getActiveFiltersCount = () => {
    ...
    if (
      relevant.showRoadType &&
      (tempFilters.houseRoadType ||
        tempFilters.landRoadType ||
        tempFilters.shopRoadType)
    )
      count++;
    if (
      relevant.showCustomArea &&
      (tempFilters.houseArea || tempFilters.landArea || tempFilters.shopArea)
    )
      count++;
  ...
  const parsePriceRange = (type, label) => {
    ...
    if (type === "Land") {
      switch (label) {
        ...
        case "1 Crore - 2 Crores":
          return [10000000, 20000000];
        case "2 Crores - 5 Crores":
  ...
  const handleApplyFilters = () => {
    ...
    const parsedRange = parsePriceRange(
                ...
                <div className="flex items-center justify-center gap-2">
                  {isLoading || isInitialLoading ? (
                    <>
                      ...
                      <span>
                        {isInitialLoading ? "Loading..." : "Searching..."}
                      </span>
                    </>
                  ) : (
                    <>
                ...
                <div className="flex flex-col items-center space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-gray-600">
                      {isInitialLoading
                        ? "Fetching all available properties from database"
                        : address?.label && address.label.trim()
                        ? `Finding properties in ${address.label
                            .split(",")[0]
                            ?.trim()}`
                        : propertyType !== "All"
                  ...
                  <div className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm font-medium">
                    {isInitialLoading
                      ...
                      ? currentAction === "Sell"
                        ...
                        : "🏘️ Looking for properties to rent"
                      : propertyType !== "All"
                      ? `🔍 Searching all ${propertyType} properties`
          ...
          <div className="container mx-auto px-4">
              ...
              (listing || []).length > 0 && (
                <div className="mb-8 bg-gray-50 border border-gray-100 rounded-lg p-4">
                  ...
                  <div className="mt-2 text-sm text-gray-600">
                    💡 Browse all properties from database • Select a property
                    type or location to filter results
                  </div>
                </div>
              )}
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-2 text-sm">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {getRelevantFilters(propertyType).showRooms &&
                  roomsCount > 0 && (
                ...
                {getRelevantFilters(propertyType).showBathrooms &&
                  bathRoomsCount > 0 && (
                ...
                {getRelevantFilters(propertyType).showParking &&
                  parkingCount > 0 && (
                ...
                {tempFilters.sortBy && tempFilters.sortBy !== "newest" && (
                  <span className="bg-gray-50 text-gray-800 px-2 py-1 rounded-md text-xs">
                    Sort:{" "}
                    {tempFilters.sortBy === "price-low"
                      ? "Price Low-High"
                      : tempFilters.sortBy === "price-high"
                      ? "Price High-Low"
                      : tempFilters.sortBy === "area-large"
                      ? "Area Large-Small"
                      : tempFilters.sortBy === "area-small"
                      ? "Area Small-Large"
                      : tempFilters.sortBy === "oldest"
                      ? "Oldest First"
                      : tempFilters.sortBy}
                  </span>
                )}
              ...
              <div className="w-full flex flex-col items-center justify-center text-center my-8 px-4">
                <p className="text-xl sm:text-2xl font-bold text-[#1a202c] mb-4 leading-snug">
                  🧐 तपाईको <span className="text-[#3b82f6]">बिशेष रोजाई</span>{" "}
                  अझ गहिरोसंग खोज्न <br />
                  ...
                  {tempFilters.propertyType === "Room/Flat" && (
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <option value="1BHK">
                          1BHK (Bedroom, Hall, Kitchen)
                        </option>
                        <option value="2BK">2BK (Two Bedrooms, Kitchen)</option>
                        <option value="2BHK">
                          2BHK (Two Bedrooms, Hall, Kitchen)
                        </option>
                  ...
                  tempFilters.roomType && (
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-700">
                            Price Range (select property type first)
                          </label>
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-gray-700">
                            पार्किङ उपलब्ध छ? (Parking Available?)
                          </label>
                          <div className="flex items-center space-x-3 mt-1">
                            <button
                              onClick={() =>
                                setTempFilters((prev) => ({
                                  ...prev,
                                  flatHasParking: !prev.flatHasParking,
                                }))
                              }
                        ...
                        {tempFilters.flatHasParking && (
                          <>
                            <div className="space-y-2">
                              <label className="text-sm font-medium text-gray-700">
                                कार पार्किङ संख्या (No. of Car Parking)
                              </label>
                              <input
                                type="number"
                                min={0}
                            ...
                            <div className="space-y-2">
                              <label className="text-sm font-medium text-gray-700">
                                बाइक पार्किङ संख्या (No. of Bike Parking)
                              </label>
                              <input
                                type="number"
                                min={0}
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            <option value="">
                              पानीको प्रकार चयन गर्नुहोस्
                            </option>
                            {!tempFilters.waterType?.includes(
                              "24 hour water on tap"
                            ) && (
                              <option value="24 hour water on tap">
                                नलबाट २४ घण्टा पानी (24hr Water)
                              </option>
                            )}
                            {!tempFilters.waterType?.includes(
                              "Storage water"
                            ) && (
                              <option value="Storage water">
                                ट्यांकी पानी (Storage)
                              </option>
                            )}
                            {!tempFilters.waterType?.includes("Inar water") && (
                              <option value="Inar water">
                                इनार/कुँडो पानी (Well)
                              </option>
                            )}
                          </select>
                          ...
                          {tempFilters.waterType &&
                            tempFilters.waterType.length > 0 && (
                              <div className="mt-3">
                                <label className="text-sm font-medium text-gray-700 mb-2 block">
                                  चयन गरिएका विकल्पहरू (Selected Options):
                                </label>
                                <div className="flex flex-wrap gap-2">
                                  {tempFilters.waterType.map(
                                    (option, index) => (
                                      <span
                                        key={index}
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            const updatedOptions =
                                              tempFilters.waterType.filter(
                                                (_, i) => i !== index
                                              );
                                            setTempFilters((prev) => ({
                                              ...prev,
                                              waterType: updatedOptions,
                                            }));
                                          }}
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-300">
                    ...
                    <div className="w-full flex flex-col sm:flex-row justify-center gap-6 mb-8">
                      ...
                      <div className="flex flex-col items-center bg-blue-50 border border-blue-200 rounded-xl p-4 shadow-md w-full max-w-xs hover:shadow-lg transition">
                        <label className="text-md font-semibold text-blue-800 mb-2">
                          शयनकक्ष (Bedrooms)
                        </label>
                      ...
                      <div className="flex flex-col items-center bg-pink-50 border border-pink-200 rounded-xl p-4 shadow-md w-full max-w-xs hover:shadow-lg transition">
                        <label className="text-md font-semibold text-pink-800 mb-2">
                          नुहाउने कोठा (Bathrooms)
                        </label>
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() =>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                        ...
                        <div className="flex items-center space-x-3">
                          <button
                            onClick={() =>
                              setTempFilters((prev) => ({
                                ...prev,
                                houseHasParking: !prev.houseHasParking,
                              }))
                            }
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      ...
                      {tempFilters.houseHasParking && (
                        <>
                          ...
                          <div className="space-y-1">
                            <label className="text-sm font-medium text-gray-700">
                              Car Parking (कार पार्किङ संख्या)
                            </label>
                            <input
                              type="number"
                              min={0}
                              ...
                              onChange={(e) =>
                                setTempFilters((prev) => ({
                                  ...prev,
                                  houseCarParking: e.target.value,
                                }))
                              }
                          ...
                          <div className="space-y-1">
                            <label className="text-sm font-medium text-gray-700">
                              Bike Parking (बाइक पार्किङ संख्या)
                            </label>
                            <input
                              type="number"
                              min={0}
                              ...
                              onChange={(e) =>
                                setTempFilters((prev) => ({
                                  ...prev,
                                  houseBikeParking: e.target.value,
                                }))
                              }
                              className="w-full px-3 py-1.5 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm"
                            />
                      ...
                      <div className="space-y-1">
                        ...
                        >
                          ...
                          <option value="lt-12">
                            ⬇ १२ फिट भन्दा कम (Less than 12 ft)
                          </option>
                          ...
                          <option value="highway">
                            हाइवे / मुख्य सडक (Highway/Main Road)
                          </option>
                        </select>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          <option value="">
                            क्षेत्रफल चयन गर्नुहोस् (Choose Area Range)
                          </option>

                          {/* Gharderi (Urban) */}
                          <option value="under-4-aana">
                            ⬇ ४ आना भन्दा कम (Less than 4 Aana)
                          </option>
                          <option value="above-4-aana">
                            ⬆ ४ आना भन्दा बढी (More than 4 Aana)
                          </option>
                          <option value="above-1-ropani">
                            ⬆ १ रोपनी भन्दा बढी (More than 1 Ropani)
                          </option>

                          {/* Terai (Madhesh) */}
                          <option value="under-1-kattha">
                            ⬇ १ कठ्ठा भन्दा कम (Less than 1 Kattha)
                          </option>
                          <option value="above-1-kattha">
                            ⬆ १ कठ्ठा भन्दा बढी (More than 1 Kattha)
                          </option>
                          <option value="above-4-kattha">
                            ⬆ ४ कठ्ठा भन्दा बढी (More than 4 Kattha)
                          </option>
                          <option value="above-1-bigha">
                            ⬆ १ बिगाहा भन्दा बढी (More than 1 Bigha)
                          </option>
                      ...
                      <div className="space-y-2">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setTempFilters((prev) => ({
                              ...prev,
                              landRoadType: e.target.value,
                            }))
                          }
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-white"
                        >
                          <option value="">
                            कुनै पनि बाटो (Any Road Type)
                          </option>
                          <option value="Pitched Road">
                            पक्की बाटो (Pitched Road)
                          </option>
                          <option value="Graveled Road">
                            ग्राभेल बाटो (Graveled Road)
                          </option>
                          <option value="Dirt Road">
                            कच्ची बाटो (Dirt Road)
                          </option>
                          <option value="Main Road">
                            मुख्य बाटो (Main Road)
                          </option>
                ...
                {tempFilters.propertyType === "Shop" && (
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                      ...
                      <div className="space-y-1">
                        <label className="text-sm font-medium text-gray-700">
                          Parking Available (पार्किङ उपलब्ध?)
                        </label>
                        <div className="flex items-center space-x-3">
                          <button
                            onClick={() =>
                              setTempFilters((prev) => ({
                                ...prev,
                                shopHasParking: !prev.shopHasParking,
                              }))
                            }
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          ...
                          <div className="space-y-1">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setTempFilters((prev) => ({
                                  ...prev,
                                  shopBikeParking: e.target.value,
                                }))
                              }
                              className="w-full px-3 py-1.5 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 text-sm"
                            />
