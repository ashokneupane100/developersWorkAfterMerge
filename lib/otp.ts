...
export async function storeOTP(email: string, otp: string) {
  try {
    ...
    if (userError || !userData || !userData.id) {
      console.warn(`User not found for email ${normalizedEmail} during OTP generation`);
    ...
    console.log(`Generating OTP for user ID: ${userId}, email: ${normalizedEmail}`);
  ...
  try {
    ...
    if (data.user_id) {
      ...
      if (userError || !userData) {
        console.error(`User ID ${data.user_id} no longer exists but has valid OTP`);
        return {
          success: false,
      ...
      if (userData.email.toLowerCase() !== normalizedEmail) {
        console.error(`Email mismatch: OTP for ${normalizedEmail}, but user_id ${data.user_id} has email ${userData.email}`);
        return {
          success: false,
          message: "Account verification failed"
        };
